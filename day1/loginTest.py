...
#5.点击登录按钮
#所有调用方法,都会有提示信息,没有提示信息就说明没有
# 6.检查登录是否成功,按照现在所学,还不能定位用户名信息
#现在自己实现一个自动化注册功能
#Alt+Enter 导包的快捷键,选择 import this name
#time.sleep() 这个方法提供了一种固定的时间等待
#这里的意义是点击登录按钮后,等5秒后,在检查用户是都正常显示
#弊端是,因为网络延迟,不着调是每次等1秒合适还是5秒合适
#解决办法:使用智能等待,替换
# tim.sleep(5)
username_text=driver.find_element_by_xpath("/html/body/div[2]/div[1]/div[1]/div[2]/a[1]").text
print(username_text)
#w我们可以通过if语句,判断页面显示的文本和预期的文本是否一致,来判断测试用例是否正常执行
if username_text=='您好 yushanshan':
     print("测试执行通过")

else:
    print ("测试执行失败")

#因为selenium主要做回归测试,所以测试脚本都是pass的 ,只有开发做了代码变更才有可能失败
#一般工作中不用if...else语句做判断,后面详细讨论
#7.点击进入商城购物按钮
#driver.find_element_by_xpath("/html/body/div[3]/div/div[2]/dl[1]/dd/div/p/a").click()
#xpath有一个缺点:定位元素的可读性比较差,有没有可读性更好的方法
...
#窗口切换
driver.close()#关闭selenium 正在公做的窗口
